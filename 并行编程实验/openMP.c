...
__attribute__((aligned(32))) double point_coordinate[ZUIDADIANSHU][2];		//存储点x y坐标
...
double distance(int x, int y) {
    // 确保索引在有效范围内
    if (x >= ZUIDADIANSHU || y >= ZUIDADIANSHU) {
        fprintf(stderr, "Index out of bounds%d\t%d\n",x,y);
        exit(EXIT_FAILURE);
    }

    // 使用 _mm256_set_pd 设置对齐的数据
    __m256d coord_x = _mm256_set_pd(0.0, 0.0, point_coordinate[x][1], point_coordinate[x][0]);
    __m256d coord_y = _mm256_set_pd(0.0, 0.0, point_coordinate[y][1], point_coordinate[y][0]);

    // 计算 x 和 y 坐标的差
    __m256d diff = _mm256_sub_pd(coord_x, coord_y);

    // 计算平方
    __m256d diff_sq = _mm256_mul_pd(diff, diff);

    // 计算平方和（仅前两项）
    double diff_sq_array[4];
    _mm256_store_pd(diff_sq_array, diff_sq);

    double sum_sq = diff_sq_array[0] + diff_sq_array[1];
...
int *simulation(int *x,int point_number)
{
	...
	int *ti = malloc(point_number*sizeof(int));
				int *tj = malloc(point_number*sizeof(int));
				memcpy(ti,i,point_number*sizeof(int));
				memcpy(tj,j,point_number*sizeof(int));

	int *ti2 = malloc(point_number*sizeof(int));
				int *tj2 = malloc(point_number*sizeof(int));
				memcpy(ti2,i,point_number*sizeof(int));
				memcpy(tj2,j,point_number*sizeof(int));
double nextlen=0.0,len=0.0,local_len=0.0,d_len=0.0;
	do
	{
		do
		{
			 ...
			 #pragma omp  parallel sections private(random, temp)
			 {
				...
				for(m=0;m<2000;m++){
				Neighbour(ti,tj,point_number);
				random = P(ti,tj,t,point_number);
				if ( (random == 1.0) || (random > random0_1()) )
			{
				temp = ti;
				ti = tj;
				tj = temp;//保证i序列始终是当前起始序列
			 }
			}
				}
				...
				for(ml=0;ml<2000;ml++){
				Neighbour(ti2,tj2,point_number);
				random = P(ti2,tj2,t,point_number);
				if ( (random == 1.0) || (random > random0_1()) )
			{
				temp = ti2;
				ti2 = tj2;
				tj2 = temp;//保证i序列始终是当前起始序列
			 }
			}
				}
 ...
 #pragma omp parallel
 {
	#pragma omp master
			 {
				random = P(i,ti,t,point_number);
				if ( (random == 1.0) || (random > random0_1()) )
			{
				temp = i;
				i = ti;
				ti = temp;//保证i序列始终是当前起始序列
			 }

			 random = P(i,ti2,t,point_number);
				if ( (random == 1.0) || (random > random0_1()) )
			{
				temp = i;
				i = ti2;
				ti2 = temp;//保证i序列始终是当前起始序列
			 }
			 }
 }
